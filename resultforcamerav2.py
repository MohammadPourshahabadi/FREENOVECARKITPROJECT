...
Usage:
  ...
  3. The script:
       ...
       - Prints which spot autopilot would choose (lowest-numbered empty)
"""
...
# Make sure current script folder is on sys.path (same trick as dashboard uses implicitly)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
if SCRIPT_DIR not in sys.path:
    sys.path.insert(0, SCRIPT_DIR)

# -------- Camera import (EXACTLY like dashboard) --------
try:
...
def main():
    ...
    if Camera is None:
        ...
        print("       Please make sure this script is in the SAME folder as Camera_1.py")
        sys.exit(1)
