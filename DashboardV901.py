...
# -------- Camera import (now using Camera_1.py) --------
try:
...
class ManualHoldDashboard:
    ...
    def toggle_camera(self):
        if not self.camera_enabled:
            if Camera is None:
                self.status.config(text="Status: Camera module (Camera_1.py) not found")
                return
            ...
            try:
                self.camera = Camera()
                # Start JPEG streaming; implementation depends on your Camera_1.py
