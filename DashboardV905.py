...
class ManualHoldDashboard:
    ...
    def update_camera_view(self):
        ...
        try:
            ...
            if frame is not None:
                # frame is RGB from Camera_1
                img = Image.fromarray(frame)

                # Optional: resize if you want a fixed larger size
    ...
    # ========== Capture image using the same camera ==========

    def capture_image(self):
        """
        Capture a single image from the current camera (if available)
        and save it into Captured_images as a JPG.
        """
        if Camera is None:
            self.status.config(text="Status: Capture error: Camera module not found")
            return

        frame = None

        try:
            # If preview camera is already running, just grab a frame
            if self.camera_enabled and self.camera is not None:
                frame = self.camera.get_frame()
            else:
                # Start a temporary camera just for capture
                temp_cam = Camera()
                temp_cam.start_stream()
                time.sleep(0.3)
                frame = temp_cam.get_frame()
                temp_cam.close()

            if frame is None:
                self.status.config(text="Status: Capture error: no frame")
                return
            ...
            # frame is RGB; convert to BGR for OpenCV saving
            bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
            cv2.imwrite(filepath, bgr)

            self.status.config(text=f"Status: IMAGE SAVED as {filename}")
        except Exception as e:
