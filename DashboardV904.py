...
import subprocess
import os
import io  # not strictly needed, but OK
...
# -------- Camera import (Camera_1.py / camera_1.py) --------
try:
...
class ManualHoldDashboard:
    def __init__(self, master):
        ...
        self.camera_photo = None  # keep reference so Tk doesn't GC it
        ...
        # Allow this frame to grow so camera can be large
        self.camera_frame.pack(pady=(4, 8), fill=tk.BOTH, expand=True)

        # No fixed width/height: it will adapt to image size
        ...
        # Let label expand with the window
        self.camera_label.pack(fill=tk.BOTH, expand=True)

        # ---------- Capture Image button ----------
        capture_frame = tk.Frame(master)
        capture_frame.pack(pady=(0, 8))
    ...
    def toggle_camera(self):
        if not self.camera_enabled:
            if Camera is None:
                self.status.config(text="Status: Camera module (Camera_1.py / camera_1.py) not found")
                return
            ...
            try:
                ...
                self.camera_label.config(text="", bg="#000000", fg="#ffffff")
    ...
    def update_camera_view(self):
        ...
        try:
            ...
            if frame is not None:
                # frame is BGR from Picamera2 -> convert to RGB for Tkinter
                rgb = frame[:, :, ::-1]
                img = Image.fromarray(rgb)

                # Use full camera resolution (e.g., 640x480).
                # If you want to force a bigger size, uncomment this:
    ...
    # ========== Capture image (runs capture_image.py once) ==========

    def capture_image(self):
        """
        Run capture_image.py one time to take a photo and save it
        into Captured_images. If the camera preview is ON, we stop
        it temporarily to avoid Picamera2 conflicts, then restart.
        """
        was_camera_on = self.camera_enabled

        # To avoid conflicts with Picamera2 being used in two processes,
        # stop our preview camera first if it's running.
        if was_camera_on:
            self.stop_camera()

        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            # Run capture_image.py once
            subprocess.run(
                ["python3", "capture_image.py"],
                cwd=script_dir,
                check=True
            )
            self.status.config(text="Status: IMAGE CAPTURED (Saved in Captured_images)")
        except subprocess.CalledProcessError as e:
            self.status.config(text=f"Status: Capture script error (exit {e.returncode})")
        except Exception as e:
            self.status.config(text=f"Status: Capture error: {e}")
        finally:
            # Optionally restart camera preview if it was ON
            if was_camera_on:
                self.toggle_camera()
