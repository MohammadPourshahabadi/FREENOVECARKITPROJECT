...
# Camera module from Freenove camera.py
try:
    from camera import Camera
except ImportError:
    Camera = None

# For showing camera frames in Tkinter
try:
...
class ManualHoldDashboard:
    def __init__(self, master):
        ...
        # External process (autopark only)
        self.autopark_process = None

        # Sensors / camera objects
        ...
        subtitle = tk.Label(
            ...
            text=(
                ...
                "Camera / Ultrasonic / Infrared switches = ON/OFF\n"
                "Auto Park = run autopark.py (disables manual)"
            ),
            font=("Arial", 10)
        )
        ...
        # ==== Toggles: Camera / Ultrasonic / Infrared / Auto Park ====
    ...
    def toggle_camera(self):
        if not self.camera_enabled:
            ...
            try:
                # Create camera and start JPEG stream
                self.camera = Camera()
                self.camera.start_stream()   # no filename -> streaming JPEG via StreamingOutput
    ...
    def update_camera_view(self):
        if not self.camera_enabled or self.camera is None:
            # ensure label shows off if we fall out
        ...
        try:
            ...
            # If something goes wrong, just show text but keep trying
